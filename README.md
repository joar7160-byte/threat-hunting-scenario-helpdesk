...
In early October, several intern and employee workstations began spawning unusual processes from their Downloads folders after an unfamiliar “helpdesk” script appeared. The affected machines shared similar executables and naming patterns, indicating a coordinated compromise. This hunt focuses on identifying the initial infection point, tracing attacker activity, and removing any persistence mechanisms established across the network.

## Key Observations

| Observation                  | Finding                                                                            |
| ---------------------------- | ---------------------------------------------------------------------------------- |
| **Initial Infection Vector** | Executables masquerading as Helpdesk tools launched from the Downloads directory   |
| **Primary Affected Host**    | `gab-intern-vm` (intern workstation)                                               |
| **Execution Method**         | PowerShell invoked with `-ExecutionPolicy` bypass                                  |
| **Defense Evasion Artifact** | `DefenderTamperArtifact.lnk` created to disable protection                         |
| **Data Collection Attempt**  | Clipboard accessed via PowerShell “Get-Clipboard” command                          |
| **Reconnaissance Commands**  | `qwinsta`, `whoami`, `wmic logicaldisk`, and `tasklist` identified                 |
| **Persistence Mechanisms**   | Scheduled task `SupportToolUpdater` and autorun registry key `RemoteAssistUpdater` |
...
```
<img width="1605" height="496" alt="image" src="https://github.com/user-attachments/assets/2b139e65-7706-45e8-a3c1-2cefdf53d56f" />
